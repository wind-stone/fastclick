;(function () {
	...
	function FastClick(layer, options) {
		 ...
		 * @type boolean
		 */
		// 仅在 touchstart 时会设置为 true，其他所有地方都设置为 false
		 ...
		 * @type number
		 */
		// 开始追踪 click 时即触发 touchstart 的时间戳
	...
	*/
	 ...
	 *
	 * 判断给定的元素是否需要原生的 click 事件，以下情况需要原生的 click 事件
	 * - 元素是 button、select、textarea、input，且处于 disabled 状态
	 * - 元素是 input，且在 iOS 平台下，且 input.type 是 file
	 * - 元素是 label、iframe、video
	 * - 元素有 needsclick 类
	 */
	...
	FastClick.prototype.sendClick = function(targetElement, event) {
		...
		// 标志这个事件对象是由 fastclick 创建的自定义事件对象
	...
	FastClick.prototype.onTouchStart = function(event) {
		...
		// 当前事件目标元素上有两个触摸点时，忽略 touchstart 事件
		...
		if (deviceIsIOS) {
			...
			// 选区的起始点和终点不在同一个位置，即框选了文本，则忽略 touchstart 事件
	...
	FastClick.prototype.onTouchMove = function(event) {
		...
		// 若触摸点已经移动，则取消 click 的追踪，即不会产生 click 事件
	...
	FastClick.prototype.onTouchEnd = function(event) {
		...
		// 若 trackingClick 为 false，则说明触摸点 move 过，不需要再触发 click 事件
		// 直接返回
		if (!this.trackingClick) {
			return true;
		}
		...
		// 若不需要原生 click 事件，则阻止 touchend 的默认行为，这样就不会产生原生的 click 事件
		// 再手动触发合成的 click 事件
